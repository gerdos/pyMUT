...
def select_best_rotemer_based_on_clashes(pdb_object, chain, res_num, mutate_to, sample_residue, rotamers, skip_own_chain=False):
    best_rotamer = None
    ...
    for rotamer in rotamers:
        ...
        for rotamer_atom, rotamer_vector in sample_residue.items():
            ...
            close_atoms = ns.search(rotamer_vector, 5)
            for close_atom in close_atoms:
                chain_if_close_atom = close_atom.get_parent().get_parent()
                if close_atom.get_parent().get_id()[1] == res_num and chain_if_close_atom.get_id() == chain:
                ...
                if dist > 6:
                    continue
                try:
                    vdw_radi = VW_RADII[close_atom.get_parent().get_resname()][close_atom.get_id()] + VW_RADII[mutate_to][
                        rotamer_atom]
                except KeyError:
                    continue
...
def mutate(pdb_obj, chain, res_num, mutate_to, rotamer_lib=None, mutation_type="best"):
    _residue, _residue_idx = \
    [(x, n) for n, x in enumerate(pdb_obj[0][chain].get_residues()) if x.get_id()[1] == res_num][0]
    # print(_residue)
    ...
    if mutate_to not in ["ALA", "GLY"]:
        ...
        if mutation_type == 'first':
            ...
            selected_rotamer = select_best_rotemer_based_on_clashes(pdb_obj, chain, res_num, mutate_to,
                                                                    sample_residue, rotamer_lib[mutate_to][phi][psi])
        elif mutation_type == "bestother":
            selected_rotamer = select_best_rotemer_based_on_clashes(pdb_obj, chain, res_num, mutate_to,
                                                                    sample_residue, rotamer_lib[mutate_to][phi][psi], skip_own_chain=True)
